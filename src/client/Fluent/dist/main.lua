	...
	TitleBar.Frame = New("Frame", {
		...
		New("Frame", {
			...
			Config.Logo and New("ImageLabel", {
				Image = Config.Logo,
				Size = UDim2.fromOffset(24, 24), -- Assuming size, user said w=420 h=420 for thumb but display should be icon size
				BackgroundTransparency = 1,
				ThemeTag = {
					ImageColor3 = "Text",
				},
			...
			New("TextLabel", {
				...
				Visible = not Config.Logo,
				LayoutOrder = 0,
			}),
			Config.Badge and New("ImageLabel", {
				Image = Config.Badge,
				Size = UDim2.fromOffset(16, 16),
				BackgroundTransparency = 1,
				ThemeTag = {
...
function Element:New(Idx, Config)
	...
	function Toggle:Keybind(Idx, Config)
		local Keybind = {
			Value = Config.Default,
			Mode = Config.Mode or "Toggle",
			Type = "Keybind",
		...
		local KeybindDisplayLabel = New("TextLabel", {
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal),
			Text = Config.Default and (Config.Default.Name or tostring(Config.Default)) or "None",
			TextColor3 = Color3.fromRGB(240, 240, 240),
			TextSize = 13,
			TextXAlignment = Enum.TextXAlignment.Center,
			Size = UDim2.new(0, 0, 1, 0),
		...
		local CancelButton = New("TextButton", {
			...
			Parent = StatusHolder,
			LayoutOrder = 1,
			Visible = (Keybind.Value ~= nil),
			ThemeTag = {
				TextColor3 = "SubText"
			}
		})

		-- Adjust ToggleSlider order to be last
		if ToggleSlider then ToggleSlider.LayoutOrder = 2 end

		local function UpdateDisplay()
			local Val = Keybind.Value
			local KeyName = "None"
			if Val then
				KeyName = (typeof(Val) == "EnumItem") and Val.Name or tostring(Val)
			end
			
			KeybindDisplayLabel.Text = KeyName
			CancelButton.Visible = (Val ~= nil and Val ~= Enum.KeyCode.Unknown and KeyName ~= "None")
		end
		...
		Creator.AddSignal(CancelButton.MouseButton1Click, function()
			Keybind.Value = nil
			UpdateDisplay()
			Library:SafeCallback(Keybind.ChangedCallback, nil)
			Library:SafeCallback(Keybind.Changed, nil)
		end)
		...
		Creator.AddSignal(KeybindDisplayFrame.InputBegan, function(Input)
			...
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				Event = game:GetService("UserInputService").InputBegan:Connect(function(Input)
					...
					if not Key or (Input.KeyCode == Enum.KeyCode.Unknown and not (Key == "MouseLeft" or Key == "MouseRight")) then
						return
					end

                    local EndedEvent
                    EndedEvent = game:GetService("UserInputService").InputEnded:Connect(function(Input)
                        ...
                        then
                            Picking = false
                            Keybind.Value = Key ~= "MouseLeft" and Key ~= "MouseRight" and Input.KeyCode or Key
                            
                            UpdateDisplay()

                            Library:SafeCallback(Keybind.ChangedCallback, Keybind.Value)
		...
		Creator.AddSignal(game:GetService("UserInputService").InputBegan, function(Input)
			if not Picking and not game:GetService("UserInputService"):GetFocusedTextBox() then
				if Keybind.Value then
					local KeyName = (typeof(Keybind.Value) == "EnumItem") and Keybind.Value.Name or Keybind.Value

					if (Input.KeyCode.Name == KeyName) or
						(KeyName == "MouseLeft" and Input.UserInputType == Enum.UserInputType.MouseButton1) or
						(KeyName == "MouseRight" and Input.UserInputType == Enum.UserInputType.MouseButton2) then
...
return Themes
    ...
    -- === FSSVS ARCADE RETRO GALAXY (BRIGHT POP-UP) ===
    Name = "FSSvs",

    Accent = Color3.fromRGB(0, 255, 255), -- Electric Cyan Accent (Max Saturation)

    -- Brighter, semi-transparent background to let particles shine but keep UI visible
    AcrylicMain = Color3.fromRGB(30, 20, 50),    -- Brighter Galaxy Purple
    AcrylicBorder = Color3.fromRGB(0, 255, 255), -- Cyan Neon Glow Border
    AcrylicGradient = ColorSequence.new(Color3.fromRGB(40, 30, 70), Color3.fromRGB(20, 10, 40)),
    AcrylicNoise = 0.96,                         -- Retro Grain

    TitleBarLine = Color3.fromRGB(255, 0, 255),  -- Magenta Neon Line
    Tab = Color3.fromRGB(220, 220, 255),         -- Very Bright Text

    -- Elements should "pop" against the dark background
    Element = Color3.fromRGB(50, 30, 70),          -- Lighter Purple Element
    ElementBorder = Color3.fromRGB(200, 0, 255),   -- Bright Neon Purple Border
    InElementBorder = Color3.fromRGB(0, 255, 255), -- Cyan Inner Border (High Contrast)
    ElementTransparency = 0.2,                     -- Less transparent to make elements pop more

    ToggleSlider = Color3.fromRGB(0, 255, 255),    -- Cyan Toggle
    ToggleToggled = Color3.fromRGB(30, 20, 50),    -- Dark Handle matching BG

    SliderRail = Color3.fromRGB(80, 40, 120),      -- Visible Purple Rail

    DropdownFrame = Color3.fromRGB(40, 30, 60),
    DropdownHolder = Color3.fromRGB(35, 25, 55),
    DropdownBorder = Color3.fromRGB(0, 255, 255), -- Cyan Border
    DropdownOption = Color3.fromRGB(0, 255, 255), -- Cyan Text

    Keybind = Color3.fromRGB(255, 0, 255),        -- Magenta Keybind

    Input = Color3.fromRGB(40, 30, 60),
    InputFocused = Color3.fromRGB(60, 40, 90),
    InputIndicator = Color3.fromRGB(255, 0, 255), -- Magenta Indicator

    Dialog = Color3.fromRGB(50, 30, 70),
    DialogHolder = Color3.fromRGB(40, 25, 60),
    DialogHolderLine = Color3.fromRGB(0, 255, 255),
    DialogButton = Color3.fromRGB(60, 40, 80),
    ...
    SubText = Color3.fromRGB(200, 190, 240), -- Bright Lavender Subtext
    Hover = Color3.fromRGB(0, 255, 255),     -- Pure Cyan Hover
    HoverChange = 0.15
}
 end,
