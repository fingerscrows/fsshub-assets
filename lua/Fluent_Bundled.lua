...
Modules["Root"] = function() -- init.lua
    ...
    function Fluent:SafeCallback(f, ...)
        if not f then return end
        local success, event = pcall(f, ...)
        if not success then
            local _, i = event:find(":%d+: ")
            if not i then return print(event) end
            return print(event:sub(i + 1))
        end
    end
    function Fluent:SetTheme(ThemeName)
        self.Creator.SetTheme(ThemeName)
        self.Creator.UpdateTheme()
    end
    function Fluent:Construct()
        return self
    end
    function Fluent:Notify(options)
        return self.Elements.Notification(options)
    end
    return Fluent
end
